...
public class NumberedDirectedGraph extends ANumberedGraph {

	/* Loop counts as 2 here */
	public NumberedDirectedGraph(int num) {
		super(num);
	}

	@Override
	public void addEdge(int p1, int p2, int value) {
		if (p1 < 0 || p1 >= this.maxVertex || p2 < 0 | p2 >= this.maxVertex)
			return;
		this.adjMatrix[p1][p2] = value;
	}
			...
			for (int column = 0; column < this.adjMatrix[row].length; column++)
				if (this.adjMatrix[row][column] > 0)
					for (int k = 0; k < this.adjMatrix[row][column]; k++) {
						System.out.printf("(%d, %d)=%d ", row, column, getEdge(row, column));
					}
		System.out.println();
	}
	...
	public boolean isConnectedGraph() {
		NumberedUndirectedGraph result = toUndirectedGraph();
		return result.isConnectedGraph();
	}

	public NumberedUndirectedGraph toUndirectedGraph() {
		NumberedUndirectedGraph result = new NumberedUndirectedGraph(maxVertex);
...
//					System.out.println(row + "::" + column + "added!");
	...
	@Override
	public ANumberedGraph copy() {
		NumberedDirectedGraph g = new NumberedDirectedGraph(maxVertex);
...
//				g.adjMatrix[i][j] = this.adjMatrix[i][j];
	...
	public LinkedList<Integer> getEulerCircuit() {
		...
		NumberedDirectedGraph graph2 = (NumberedDirectedGraph) copy();
		int i = 0;
		circuit.add(0);
	...
	public LinkedList<Integer> getEulerPath() {
		...
		NumberedDirectedGraph a = (NumberedDirectedGraph) copy();
